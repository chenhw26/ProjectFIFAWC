...
void KonckoutStage:: scheduling16(const vector<string> &venues){
	...
	for(int i = 0; i < 8; i++){
		...
		cur_match.teamA = top16[2 * i];
		cur_match.teamB = top16[2 * i + 1];
...
void KonckoutStage:: schedulingQuarter(const vector<string> &venues){
	...
	for(int i = 0; i < 4; i++){
		...
		cur_match.teamA = top8[2 * i];
		cur_match.teamB = top8[2 * i + 1];
...
void KonckoutStage:: schedulingSemi(const vector<string> &venues){
	...
	for(int i = 0; i < 2; i++){
		...
		cur_match.teamA = top4[2 * i];
		cur_match.teamB = top4[2 * i + 1];
...
void KonckoutStage:: schedulingFinal(const vector<string> &venues){
	...
	for(int i = 0; i < 2; i++){
		...
		cur_match.date = 16 - i;
		cur_match.venue = venues[i];
		cur_match.teamA = FinalTeams[2 * i];
		cur_match.teamB = FinalTeams[2 * i + 1];
...
void KonckoutStage::playing16(Result& result){
	...
	for(int i = 0; i < Round16.size(); i++){
		...
		showMatch(out, "", Round16[i].teamA.country, Round16[i].teamB.country, score, penalty);
	}
	out.close();
}
void KonckoutStage::playingQuarter(Result& result){
	...
	for(int i = 0; i < Quarter_finals.size(); i++){
		...
		showMatch(out, "", Quarter_finals[i].teamA.country, Quarter_finals[i].teamB.country, score, penalty);
	}
	out.close();
}
void KonckoutStage::playingSemi(Result& result){
	...
	for(int i = 0; i < Semi_finals.size(); i++){
		...
		showMatch(out, "", Semi_finals[i].teamA.country, Semi_finals[i].teamB.country, score, penalty);
	}
	out.close();
}
void KonckoutStage:: playingFinal(Result& result){
	...
	for(int i = 1; i >= 0; i--){
		...
		showMatch(out, message[i], Final[i].teamA.country, Final[i].teamB.country, score, penalty);
	}
	out.close();
}
void KonckoutStage::showMatch(ostream& out, const string message, const string nameA, 
	...
	if(penalty != {0, 0}) out << '(' << penalty.first << ')';
	out << " VS " << score.second;
	if(penalty != {0, 0}) out << '(' << penalty.second << ')';
	out << ' ' << nameB;
}
